...
全サービスは `grafana/otel-lgtm` に OTLP で直接テレメトリを送信しています。`otel-lgtm` は OTel Collector、Tempo（トレース）、Prometheus（メトリクス）、Loki（ログ）、Grafana をオールインワンで提供します。Kong Gateway 自体も `opentelemetry` プラグインでトレース・ログ・メトリクスを送信します。

> **注意:** `otel-lgtm` はカスタム OTel Collector 設定を持たないため、Kong のステータス API (`kong:8100`) からの Prometheus スクレイプは行われません。Kong AI 系メトリクスが必要な場合は、別途 Prometheus を追加して設定してください。
