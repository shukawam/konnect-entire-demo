...
1. **AI Proxy Advanced** — OpenAI GPT-4o-mini への LLM ゲートウェイを提供（`/ai/v1` = 一問一答向け・キャッシュあり、`/ai/agent/v1` = Agent 内部の LLM 呼び出し・キャッシュなし、`/ai/agent-chat/v1` = Agent の入出力境界・キャッシュあり）
