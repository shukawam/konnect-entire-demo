...
| レイヤー           | 技術                                                                     |
| ------------------ | ------------------------------------------------------------------------ |
| フロントエンド     | Next.js 15.3, React 19, @vercel/otel                                     |
...
```bash
# GHCR から事前ビルド済みイメージを取得（初回または更新時）
docker compose pull
...
全サービスは `grafana/otel-lgtm` に OTLP で直接テレメトリを送信しています。`otel-lgtm` は OTel Collector、Tempo（トレース）、Prometheus（メトリクス）、Loki（ログ）、Grafana をオールインワンで提供します。Kong Gateway 3.14 以降では AI 関連メトリクスも `opentelemetry` プラグイン経由で OTLP 送信されます。
...
│   ├── mysql/               # DB 初期化 SQL
│   └── observability/       # grafana/otel-lgtm 用ダッシュボード設定
...
開発時にソースコードの変更を即時反映したい場合は、Docker 経由ではなくホスト上で `npm run dev:*` を直接実行してください（各サービスは tsx watch / next dev で起動します）。Docker Compose 経由のサービスは事前ビルド済みイメージで動作するため、ホットリロードはサポートされません。
