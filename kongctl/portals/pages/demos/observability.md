...
description: "Tempo・Loki・Prometheus・Grafana による統合監視を体験"
page-layout:
...
type: "info"
show-icon: true
message: "対象: SRE / Platform Engineer / オブザーバビリティに関心のある方 ｜ 所要時間: 15〜20分"
---
::
...
```sh
全サービス ──(OTLP)──> otel-lgtm :4318
                              │  (内蔵 OTel Collector)
                    ┌─────────┼─────────┐
                    v         v         v
                 Tempo    Prometheus   Loki
              (トレース)   (メトリクス) (ログ)   ← すべて otel-lgtm コンテナ内部
                    │         │         │
                    └─────────┼─────────┘
                              v
                        Grafana :3010
...
1. http://localhost:3000 にアクセスしてログイン（`jack@example.com` / `password`）
...
curl は curl 向けの API キー経路 `/admin/api/*`（`key-auth`）を使います。`X-User-Id` は Kong が `curl-admin` として自動注入するため指定不要です。
...
- 構造化 JSON ログが otel-lgtm（内蔵 OTel Collector）経由で Loki に集約される
