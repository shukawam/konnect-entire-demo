...
1. **AI Proxy Advanced** — OpenAI GPT-4o-mini への LLM ゲートウェイを提供（`/ai/v1` = 一問一答向け・キャッシュあり、`/ai/agent/v1` = Agent 専用・キャッシュなし）
...
4. **AI Semantic Cache** — 意味的に近い応答を再利用しコスト/レイテンシを削減（`/ai/v1` のみ。マルチターン会話の文脈を壊さないよう Agent 経路には掛けない）
