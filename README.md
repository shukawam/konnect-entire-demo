...
Keycloak の realm（クライアント・ユーザー）は **自分で作成してエクスポート** し、
`config/keycloak/<realm>-realm.json` に配置すると起動時に自動インポートされます。
...
2. realm `jungle-store`、NextAuth 用クライアント、デモユーザーを作成
   （リダイレクト URI 等の要件は [`config/keycloak/README.md`](config/keycloak/README.md) 参照）
3. クライアントの secret を `.env` の `AUTH_KEYCLOAK_SECRET` に設定
4. realm をエクスポートして `config/keycloak/jungle-store-realm.json` として保存
