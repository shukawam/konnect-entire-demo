...
- [mise](https://mise.jdx.dev/) (the runner for `mise run setup`; also loads `.env`)
- CLIs used by `mise run setup`: `deck` (decK) / `kongctl` / `jq` / `yq` / `openssl` (install per each tool's docs if missing)
- Docker / Docker Compose
- Node.js 20+ (for local development)
- Kong Konnect account + Personal Access Token (set in `.env` as `DECK_KONNECT_TOKEN`; shared by deck and kongctl, which mise supplies to both by reading `.env`)
- `DECK_OPENAI_API_KEY` in `.env` (for AI Gateway; alongside `DECK_KONNECT_TOKEN` it is an external secret — the rest of the dynamic/secret values are auto-generated/auto-extracted by `mise run setup`)

> Cluster certificates are auto-generated by `mise run certs:gen` (included in `mise run setup`): it creates self-signed certs and pins them declaratively on the Konnect side. See [Quick Start](#quick-start) for details.
...
**When using `mise run setup` (recommended), you only set these two by hand:**

| Variable              | Description                                                                   | Example    |
| --------------------- | ----------------------------------------------------------------------------- | ---------- |
| `DECK_KONNECT_TOKEN`  | Konnect Personal Access Token (shared by deck and kongctl); issued in Konnect | `kpat_...` |
| `DECK_OPENAI_API_KEY` | OpenAI API key (for AI Gateway)                                               | `sk-...`   |

The rest are written to `.env` automatically by `mise run setup` (`env:patch`) — you don't need to fill them in:

| Variable                          | How it is set                                                                |
| --------------------------------- | ---------------------------------------------------------------------------- |
| `PREFIX`                          | Derived from the Konnect Gateway Control Plane endpoint                      |
| `EVENT_GATEWAY_CP_ID`             | Derived from the Konnect Event Gateway ID                                    |
| `DECK_KONNECT_CONTROL_PLANE_NAME` | Control Plane name (prefixed when `RESOURCE_PREFIX` is set)                  |
| `AUTH_SECRET`                     | Auto-generated via `openssl rand -base64 32` (existing value kept)           |
| `AUTH_KEYCLOAK_SECRET`            | Auto-extracted from the client secret in `config/keycloak/realm-export.json` |

Other variables (MySQL, Kafka, service URLs, Keycloak URLs/realm, `AUTH_KEYCLOAK_ID`, etc.) work with their default values.

> For a manual setup (without `mise run setup`), set the auto-filled values (`PREFIX` / `EVENT_GATEWAY_CP_ID` / `DECK_KONNECT_CONTROL_PLANE_NAME` / `AUTH_SECRET` / `AUTH_KEYCLOAK_SECRET`) in `.env` yourself. `PREFIX` is the prefix of `<prefix>.us.cp.konghq.com`, `AUTH_SECRET` comes from `openssl rand -base64 32`, and `AUTH_KEYCLOAK_SECRET` is the realm's client secret.

### Kong Konnect Certificates

Self-signed cluster certificates are auto-generated by `mise run certs:gen` (part of `mise run setup`). They are created as `cluster.crt` / `cluster.key` under `certs/kong-gateway/` (for Kong Gateway) and `certs/event-gateway/` (for Event Gateway), and pinned declaratively on the Konnect side via kongctl's declarative config (`data_plane_certificates`).

To keep using existing certificates (e.g. Konnect-issued ones), place files of the same name under `certs/kong-gateway/` and `certs/event-gateway/` before running `certs:gen`, and they will be reused as-is (`certs/` is gitignored and not committed).
...
Prerequisite: set `DECK_KONNECT_TOKEN` (Konnect PAT) and `DECK_OPENAI_API_KEY` in `.env`. (mise reads `.env` and supplies the PAT to both deck and kongctl.)

```bash
mise run setup
```

This runs, in order: `doctor` (prerequisite checks) → `certs:gen` (generate self-signed certs) → `konnect:sync` (sync Konnect resources + pin certs) → `env:patch` (write `PREFIX` / `EVENT_GATEWAY_CP_ID` / secrets into `.env`) → `gateway:sync` (deck syncs Gateway config to the Control Plane) → `up` (`docker compose up -d --build`). Each task can also run individually (e.g. `mise run doctor`) and is idempotent on re-runs.

Isolated launch (to run E2E checks without colliding with production resources):

```bash
RESOURCE_PREFIX=e2e mise run setup
RESOURCE_PREFIX=e2e mise run teardown  # cleanup (delete Konnect resources + compose down -v)
```
...
1. **AI Proxy Advanced** — Provides an LLM gateway to OpenAI GPT-4o-mini (`/ai/v1` for one-shot Q&A with caching, `/ai/agent/v1` for the Agent's internal LLM calls without caching, `/ai/agent-chat/v1` for the Agent's input/output boundary with caching)
...
4. **AI Semantic Cache** — Reuses semantically-similar responses to cut cost/latency (applied to `/ai/v1` and the Agent's input/output boundary `/ai/agent-chat/v1`; not applied to the Agent's internal LLM calls at `/ai/agent/v1`, to avoid breaking multi-turn conversation context)
